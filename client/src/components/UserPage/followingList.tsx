...
import { useGetFollowingListQuery } from "../../generated/graphql";
...
export const FollowingList: React.FC = () => {
  ...
  const [{ data, fetching, error }] = useGetFollowingListQuery({
    variables: { id: userId },
  });
  if (error) throw new Error(error.message);
  const user = data?.getUser;
  const followings = data?.getFollowings;
  const followingUsers = followings?.map(({ following }) => ({
    id: following.id,
    name: following.name,
    bio: following.profile?.bio as string,
  }));
  ...
  return (
    <Body>
      <UserHeader>
        ...
        <HeaderInfo>
          <HeaderName>
            {user.name} (@{userId}) さんがフォローしているアカウント
          </HeaderName>
        </HeaderInfo>
      </UserHeader>
      {followingUsers && <UserList users={followingUsers} />}
    </Body>
  );
};
